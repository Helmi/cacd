...
function parseDroidMessages(sessionFilePath: string): ConversationMessage[] {
...
export class DroidAdapter extends BaseAgentAdapter {
	...
	override async findSessionFile(
		...
		const roots = [
			homePath('.droid'),
			homePath('.config', 'droid'),
			path.join(worktreePath, '.droid'),
	...
	override async parseMessages(
		...
		return parseDroidMessages(sessionFilePath);
	}

	override async extractMetadata(
		sessionFilePath: string,
	): Promise<SessionFileMetadata> {
		const messages = parseDroidMessages(sessionFilePath);
