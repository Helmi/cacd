...
export class CoreService extends EventEmitter {
    ...
    private setupServiceListeners() {
        ...
        this.sessionManager.on('sessionCreated', (session) => this.emit('sessionCreated', session));
        this.sessionManager.on('sessionExit', (session) => this.handleSessionExit(session));
        this.sessionManager.on('sessionStateChanged', (session) => this.emit('sessionStateChanged', session));
        this.sessionManager.on('sessionData', (session, data) => this.emit('sessionData', session, data));
        this.sessionManager.on('sessionDestroyed', (session) => this.emit('sessionDestroyed', session));
    }
    ...
    public async selectProject(project: GitProject): Promise<void> {
        ...
        this.sessionManager = globalSessionOrchestrator.getManagerForProject(project.path);
    ...
    public async refreshWorktrees(): Promise<Either.Either<Worktree[], AppError>> {
        const result = await Effect.runPromise(Effect.either(this.worktreeService.getWorktreesEffect()));
